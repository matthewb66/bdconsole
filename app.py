                ...
                dbc.Spinner(
                    dbc.Tabs(
                        [
                            ...
                            dbc.Tab(  # SUMMARY TAB
                                summary.create_summary_tab(bd, df_comp, df_vuln, '', '', ''),
                                label="Summary",
                                tab_id="tab_summary", id="tab_summary",
                                disabled=True,
                            ),
...
@app.callback(
    [
        ...
        Output("tab_summary", "children"),     # 2
        Output("tab_summary", "disabled"),     # 3
        Output("tab_comps", "children"),     # 4
        Output("tab_comps", "disabled"),     # 5
        Output("tab_comps", "label"),        # 6
        Output("tab_vulns", "children"),     # 7
        Output("tab_vulns", "disabled"),     # 8
        Output("tab_vulns", "label"),        # 9
        Output("tab_snippets", "children"),  # 10
        Output("tab_snippets", "disabled"),  # 11
        Output("tab_snippets", "label"),     # 12
        Output("tab_trend", "disabled"),     # 13
        Output("tab_trend", "children"),     # 14
        # Output("tab_actions", "disabled"),          # ACTIONS tab
        # Output("actions_projver", "children"),      # ACTIONS tab
        # Output("spdx_file", "value"),               # ACTIONS tab
        Output('vername', 'data'),           # 15
        Output('projverurl', 'data'),        # 16
...
def cb_vertable(row, verdata, projname):
    ...
    return vers.create_vercard(verdata[row[0]], df_comp_new, vername, projname), \
        summary.create_summary_tab(bd, df_comp_new, df_vuln, projname, vername, projverurl), False, \
...
#     ]
# )
# def cb_snipactions(snip_selected_clicks, snip_all_clicks,
#                    # action,
...
#     ]
# )
# def cb_compactions(comp_selected_clicks, comp_all_clicks,
#                    # action,
#                    origdata, vdata, selected_rows, projverurl):
#     global bd
#
...
# )
# def cb_vulnactions(
#         vuln_selected_clicks,
#         vuln_all_clicks, reload,
#         # action,
#         origdata, vdata, selected_rows, projverurl):
#     global bd
#     global vulns_json
...
def cb_trend(button, purl, vername):
    ...
    compdata, vulndata, scans, vulns, vuln_origs = trend.proc_journals(bd, purl, vername, -1)
    if compdata is None:
        return '', ''
