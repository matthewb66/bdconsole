...
def proc_events(eventlist):
    ...
    for event in eventlist:
        ...
        if evtype == 'comp':
            ...
            timelist_vulns.append(
                {
                    'timestamp': event['timestamp'],
                    'Unique Vulns': len(vuln_list),
        ...
        if evtype == 'vuln':
            ...
            vulns.append(event['vuln'])
            # vuln_origs.append(event['vulnorig'])
    ...
    return timelist_comps, timelist_vulns, scans, vulns, vuln_origs


def proc_journals(bd, projverurl, pjvername, days):
    ...
    verjournalurl = "{}/versions/{}?sort=timestamp%20desc".format(projjournalurl, arr[7])
    projjournalurl = projjournalurl + "?sort=timestamp%20desc"
    # response = hub.execute_get(verjournalurl, custom_headers=headers)
    #
    # if not response.ok:
    #     return None, None, None
    # jsondata = response.json()
    jsondata = utils.get_json(bd, verjournalurl, headers=headers)
    ...
    projconf = utils.get_json(bd, projurl)
    ...
    if 'projectLevelAdjustments' in projconf and projconf['projectLevelAdjustments']:
        ...
        jsondata = utils.get_json(bd, projjournalurl)
